...
class YOLOv5Head(nn.Module):
    def __init__(self, num_classes: int, anchors: list[list[int]], strides: list[int]) -> None:
        ...
        self.anchors /= torch.FloatTensor(strides).view(-1, 1, 1)  # ty: ignore
    ...
    def forward(self, outs: list[Tensor]) -> Tensor | list[Tensor]:
        ...
        for i in range(3):
            ...
            if self.training:
                ...
                xy = (xy * 2 + self.grid[i]) * self.strides[i]
        ...
        if self.training:
            ...
            return torch.concat(z, 1)  # (bs, 3 * na * ny * nx, 85)
    ...
    def _make_grid(self, nx: int, ny: int, i: int) -> tuple[Tensor, Tensor]:
        ...
        anchor_grid = (self.anchors[i] * self.strides[i]).view((1, self.na, 1, 1, 2))
